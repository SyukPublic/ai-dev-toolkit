...
- **`engineering-paved-path:react-best-practices`** → component purity, hooks misuse, memoization, keys,
  conditional rendering, render factories.
- **`engineering-paved-path:next-best-practices`** → App Router file conventions, RSC mechanics, metadata,
...
## How this applies to a typical Next.js client package

A client package built on Next 15 (App Router) + React 19 + TanStack Query — exactly this
skill's target stack — gets the rules directly: feature-based structure inside the client
package, server/client boundary placement (`'use client'` at leaves, data/secrets on the
server), and **server state via TanStack Query** rather than a global store. The skill stays
vendor-agnostic; treat this only as an orientation note for where it lands in a host project.
(Sibling skills `engineering-paved-path:next-best-practices` and
`engineering-paved-path:react-best-practices` cover the framework mechanics and runtime rules
referenced above.)
