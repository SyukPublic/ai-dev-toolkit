...
- `engineering-paved-path:react-best-practices` → component purity, hooks misuse, memoization, keys, derive-don't-store.
- `engineering-paved-path:next-best-practices` → App Router file conventions, RSC mechanics, metadata, image/font optimization.
...
- Don't sync state with `useEffect` — derive it. (Detail lives in `engineering-paved-path:react-best-practices`; relevant here because it dictates that derived values get *no* storage location at all.)
