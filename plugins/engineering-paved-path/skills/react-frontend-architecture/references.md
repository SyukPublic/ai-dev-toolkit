...
Sources backing the `react-frontend-architecture` skill. Focus: **where code lives and how it is split** for **React 19 + Next.js 15 (App Router)**, across project scales (small → enterprise). Component runtime correctness lives in the sibling `engineering-paved-path:react-best-practices` skill; App Router/RSC/data-fetching mechanics live in `engineering-paved-path:next-best-practices`. This skill is intentionally about **project structure, file organization, and code-placement conventions**.
...
- `engineering-paved-path:react-best-practices` → component purity, hooks misuse, memoization, keys, conditional rendering, render factories. **Not duplicated here.**
- `engineering-paved-path:next-best-practices` → App Router file conventions, RSC boundaries, metadata, image/font optimization. **This skill references structure only where it informs placement decisions.**
