...
One ❌ Bad / ✅ Good pair per rule in [SKILL.md](SKILL.md), grounded in the
reference monorepo layout from SKILL.md. The Good column reflects the pattern
to follow; the Bad column is the violation to catch in review.
...
```ts
// modules/reviews/routes.ts
import { RunRequest } from '@app/shared';
...
```ts
// reviewer-core/src/review/run.ts
import type { LLMProvider, Review, UnifiedDiff } from '@app/shared'; // → center
```
Allowed: `server → reviewer-core → @app/shared`, and `server → @app/shared`.
`@app/shared` imports only `zod` and its own contracts.
