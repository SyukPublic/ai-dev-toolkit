...
> Captures the research, how Onion maps onto a reference TypeScript backend, the 9 rules to encode,
...
## How it maps onto the reference backend layout (the skill codifies & enforces it)

| Onion layer | Where in the reference layout | Tool |
|---|---|---|
...
2. **Adapters only behind interfaces** (CRITICAL) — services depend on interfaces in `@app/shared/adapters.ts`, never on concrete classes.
...
8. **Cross-package import direction** (a `reviewer-core → server` back-edge is CRITICAL — a rule 1 purity break; other direction issues HIGH) — `reviewer-core` never imports `server`; `@app/shared` imports nothing runtime.
