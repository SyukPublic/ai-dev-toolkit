...
(e.g., contract shape, query semantics, React component behaviour), load the
relevant skill via the `Skill` tool at that point:
- Server routes / services / repositories → `engineering-paved-path:fastify-best-practices`
- Client UI → `engineering-paved-path:react-best-practices`, `engineering-paved-path:next-best-practices`
- Architecture layer placement → `engineering-paved-path:onion-architecture`
- Other surfaces (shared contracts/validation, ORM, …) → any project-provided
  skill for that surface, if the host project ships one
