...
`engineering-paved-path:typescript-expert`, `engineering-paved-path:security`)
are preloaded; load the surface-specific skills on demand with the `Skill` tool
when you design a phase that touches that surface (see the table below). You
design across surfaces, so invoke whichever apply — just not all at once up
front.
...
When you design a phase that touches a surface, invoke that surface's skills
with the `Skill` tool first (the always-on
`engineering-paved-path:onion-architecture` /
`engineering-paved-path:typescript-expert` / `engineering-paved-path:security`
are already loaded). Then label each phase in the plan with the surface it
touches so implementers apply the same subset.
...
| Plan diagrams (optional) | `sdd-engineering:mermaid-diagram` — when a flow / architecture / ER diagram clarifies the plan |
...
4. **Design.** Decide where each piece of logic belongs following the host
   project's architecture conventions (e.g. Onion Architecture — dependencies
   point inward; routes thin, services pure, adapters at the edge; see
   `engineering-paved-path:onion-architecture`). Before detailing a phase,
   invoke that surface's skills via `Skill` (per the table above) so the
