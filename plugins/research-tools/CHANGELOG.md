...
- Interview discipline runs in both directions. A prompt with a subject but no decision ("let's talk about the export stuff") comes back as at most four questions, each carrying its own default so the caller can confirm in one word. A prompt that names the decision, names the options, or states a decision already made and asks for the ADR is **not** an interview — the agent proceeds, and states its assumptions inline instead of stopping. Ask-versus-proceed is settled before the first tool call, so a discussion never spends its fact-finding and then asks anyway.
